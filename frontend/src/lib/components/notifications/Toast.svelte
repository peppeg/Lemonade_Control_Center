<script lang="ts">
  ...
  async function openToastTarget() {
    ...
    await goto(toast.href);
  }
</script>

<div class="w-full max-w-sm rounded border bg-[#171a19] p-4 shadow-2xl shadow-black/35 transition-all duration-150 {borderClass} {toast.href ? 'cursor-pointer hover:bg-[#1e211f]' : ''} {toast.exiting ? 'translate-x-4 opacity-0' : 'translate-x-0 opacity-100'}">
