...
<div class="space-y-6">
  <section class="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
    ...
    <div class="flex items-center gap-2 rounded border border-[#444936] bg-[#202321] p-1">
      <span class="rounded bg-[#111312] px-4 py-2 text-sm">Stats</span>
      <span class="px-4 py-2 text-sm text-muted-foreground">Logs</span>
      <span class="flex items-center gap-2 px-4 py-2 text-sm text-muted-foreground">
        <span class="h-2 w-2 rounded-full bg-status-off"></span>
        Live
      </span>
    </div>
  </section>
  ...
  <section class="ops-panel overflow-hidden">
    ...
    <div class="ops-terminal min-h-[360px] overflow-auto p-4">
      {#if loading}
